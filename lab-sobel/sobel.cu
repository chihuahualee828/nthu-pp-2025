...
/* Hint 7 */
// this variable is used by device, store in device constant memory
...
// void sobel (unsigned char* s, unsigned char* t, unsigned height, unsigned width, unsigned channels) {
    ...
    // global index: blockIdx: block idx within the grid; blockDim: number of threads per block; threadIdx: thread idx within the block
    int idx = blockIdx.x * blockDim.x + threadIdx.x; // start idx of each thread, since 1-D, so only x

    // gridDim.x: number of blocks within the grid
    for (int pixel = idx; pixel < height * width; pixel += gridDim.x * blockDim.x) {
        y = pixel / width; // since we flatten 2D to 1D
        x = pixel % width;
    // for (y = 0; y < height; ++y) {
    //     for (x = 0; x < width; ++x) {
            for (i = 0; i < MASK_N; ++i) {
                ...
                // 2(Gx&Gy)*3(RGB)=6 values to store convolution results
                val[i*3+2] = 0.0;
                val[i*3+1] = 0.0;
                val[i*3] = 0.0;
                
                // convolution: iterate around neighboring pixels
                for (v = -yBound; v < yBound + adjustY; ++v) {
                    for (u = -xBound; u < xBound + adjustX; ++u) {
                        if ((x + u) >= 0 && (x + u) < width && y + v >= 0 && y + v < height) { // check boundary
            ...
            double totalG = 0.0;
            double totalB = 0.0;
            // combine Gx and Gy to Gx^2 + Gy^2
            for (i = 0; i < MASK_N; ++i) {
...
int main(int argc, char** argv) {
    ...
    cudaMemcpy(device_s, host_s, height * width * channels * sizeof(unsigned char), cudaMemcpyHostToDevice); // cudaMemcpy(destination, source, ...)


    /* Hint 3 */
    // acclerate this function
    // sobel(host_s, host_t, height, width, channels);
    // assign kernel to blocks and threads(<=1024)
    sobel<<<1000,1024>>>(device_s, device_t, height, width, channels);  // 1000*1024 threads for this kernel
