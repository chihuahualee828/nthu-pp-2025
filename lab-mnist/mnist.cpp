...
void LinearLayer(float *A, float *B, float *C, float *D, int n, int k, int m) {
    #pragma acc enter data create(D[0:n*m]) // declare space of D on device, layer1_output is empty at first
...
void Sigmoid(float *A, int n, int m) {
    #pragma acc parallel loop collapse(2) copyout(A[0:n*m]) // tile only for nested
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) { 
            // Sigmoid(x) = 1/(1+exp(x))
